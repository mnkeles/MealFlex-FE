...
const navClass = ({ isActive }: { isActive: boolean }) =>
  `rounded-xl px-3 py-2 text-sm font-bold transition ${isActive ? "bg-primary-50 text-primary-700" : "text-slate-600 hover:bg-slate-100 hover:text-ink"}`;
...
export default function Navbar() {
  ...
  return (
    <>
        ...
        <div className="mx-auto flex h-16 max-w-7xl items-center gap-3 px-4 sm:px-6 lg:px-8">
          <Link to="/" className="shrink-0" aria-label="MealFlex ana sayfa">
            <MealFlexLogo
              ...
              wordmarkClassName="hidden text-xl font-black tracking-tight text-primary-600 sm:inline"
            />
          </Link>
            ...
            {addresses.length ? (
              <>
                <select
                  ...
                  className="h-11 w-full appearance-none truncate rounded-xl border border-slate-200 bg-slate-50 pl-9 pr-9 text-sm font-bold text-slate-800 outline-none transition hover:border-primary-200 focus:border-primary-400 focus:ring-4 focus:ring-primary-100"
                >
              ...
              <Link
                to="/addresses"
                className="flex h-11 items-center rounded-xl border border-primary-200 bg-primary-50 pl-9 text-sm font-bold text-primary-700 transition hover:bg-primary-100"
              >
          ...
          >
            ...
            <NavLink
              ...
              className={({ isActive }) =>
                `relative rounded-xl p-2 transition ${isActive ? "bg-primary-50 text-primary-700" : "text-slate-600 hover:bg-slate-100"}`
              }
            >
            ...
            <div className="relative" ref={profileMenuRef}>
              <button
                ...
                aria-haspopup="menu"
                className="flex items-center gap-2 rounded-xl px-3 py-2 text-sm font-bold text-slate-700 hover:bg-slate-100"
              >
              ...
              {profileOpen && (
                <div
                  ...
                  className="absolute right-0 mt-2 w-64 rounded-2xl border border-slate-200 bg-white p-2 shadow-floating"
                >
      ...
      <nav
        className="fixed inset-x-0 bottom-0 z-50 grid grid-cols-5 border-t border-slate-200 bg-white/95 px-2 pb-[max(.5rem,env(safe-area-inset-bottom))] pt-2 shadow-[0_-6px_20px_rgba(15,23,42,.06)] backdrop-blur md:hidden"
        ...
        {mobileLinks.map((item) => {
          ...
          return (
            <Link
              ...
              className={`relative flex min-h-11 flex-col items-center justify-center gap-0.5 rounded-lg py-1 text-[10px] font-bold ${active ? "text-primary-600" : "text-slate-500"}`}
            >
              <Icon className="h-5 w-5" />
              {item.to === "/notifications" && !!unread?.count && (
                <span className="absolute left-1/2 top-0 ml-2 grid h-5 min-w-5 place-items-center rounded-full bg-primary-600 px-1 text-[9px] font-black text-white">
