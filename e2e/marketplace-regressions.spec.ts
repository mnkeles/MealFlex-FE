...
test('satıcı iki ayrı aralığı çakışmaları tekrarlamadan kaydeder ve yeniden açınca görür', async ({ page }) => {
  ...
  await page.getByLabel('Başlangıç saati', { exact: true }).selectOption('12:00')
  await page.getByLabel('Bitiş saati', { exact: true }).selectOption('14:00')
  await page.getByRole('button', { name: 'Aralığı ekle' }).click()
  await page.getByRole('button', { name: 'Aralığı ekle' }).click()
  await page.getByLabel('Başlangıç saati', { exact: true }).selectOption('18:00')
  await page.getByLabel('Bitiş saati', { exact: true }).selectOption('20:00')
  await page.getByRole('button', { name: 'Aralığı ekle' }).click()
  ...
  await page.getByLabel('Başlangıç saati', { exact: true }).selectOption('20:00')
  await page.getByLabel('Bitiş saati', { exact: true }).selectOption('12:00')
  await page.getByRole('button', { name: 'Aralığı ekle' }).click()
