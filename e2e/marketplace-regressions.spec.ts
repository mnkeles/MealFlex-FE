...
test('mağaza ayarları taslağı korunur ve mesafe kaydı yalnız sunucudaki mağaza verisini kullanır', async ({ page }) => {
  ...
  await page.getByText('Sağlıklı', { exact: true }).click()
  await page.getByText('Vegan', { exact: true }).click()
  await page.getByText('Izgara', { exact: true }).click()
  await expect(page.getByText('5 / 5 seçildi')).toBeVisible()
  await expect(page.getByLabel('Sulu Yemek')).toBeDisabled()
